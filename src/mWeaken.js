/** @param {NS} ns */
export async function main(ns) {
	let [target, t = null] = ns.args
	const server = ns.getServer(target)

	while(true) {
		let home = ns.getServer()
		let {maxRam, ramUsed} = home
		let availableRam = maxRam - ramUsed

		let scriptCost = ns.getScriptRam('weaken.js')
		let threads = t !== null ? t :Math.floor(availableRam / scriptCost)
		let time = Math.ceil(ns.getWeakenTime(target)) + 100
		ns.exec('weaken.js', home.hostname, {threads: threads - 1}, target, threads - 1)
		await ns.sleep(time)
	}
}